...
def main():
	...
	for i in range(dynamic_k):
		...
		print("- Dynamic Topic: %s" % dynamic_topic_label )
                f = open(DIR + dynamic_topic_label + '.csv', 'w')
		# create table header
		header = ["Rank", "Overall"]
                custom_header = []
		for t in all_tracked_topics[i]:
			...
			while field in header:
				...
				field = "Window %d(%d)" % (t[0],suffix)
                        custom_header.append(str(t[0])+'-'+str(suffix))
			header.append( field )
		...
		for label in header[1:]:
			tab.align[label] = "l"
		# add the term rows
                print 'pos:', options.top
                print 'cs:', len(custom_header)
		for pos in range(options.top): 
			...
			tab.add_row( row )
                ...
                for x in all_tracked_topics[i]:
                    ...
                    print x
                    print ','.join(x[1])
                    f.write(str(x[0]) + '-' + str(count) + ',')
                    f.write(','.join([y.encode('utf-8') for y in map(str, x[1])] ))
                    f.write('\n')
